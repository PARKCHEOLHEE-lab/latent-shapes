// Marching Cubes.
//
// Tables and algorithm are Paul Bourke's classic implementation
// (http://paulbourke.net/geometry/polygonise/), JS port by Mikola Lysenko
// (github.com/mikolalysenko/isosurface, MIT). Vendored verbatim below; only the
// module wrapper is changed to accept a precomputed scalar field array instead
// of a potential(x, y, z) callback.
...
const triTable = [
  ...
  [0, 9, 1, 11, 6, 7], [7, 8, 0, 7, 0, 6, 3, 11, 0, 11, 6, 0], [7, 11, 6], [7, 6, 11],
  [3, 0, 8, 11, 7, 6], [0, 1, 9, 11, 7, 6], [8, 1, 9, 8, 3, 1, 11, 7, 6], [10, 1, 2, 6, 11, 7],
...
// Verbatim Lysenko polygonizer (potential-callback form). x0Start/x0End restrict
// the CELL range on the first axis so completed slabs can be polygonized
// incrementally while later layers are still being computed.
function polygonize(dims, potential, bounds, x0Start = 0, x0End = dims[0] - 1) {
  if (!bounds) {
    bounds = [[0, 0, 0], dims];
  }
  const scale = [0, 0, 0];
...
// Extract the level-set isosurface of a precomputed scalar field.
// dims = [d0, d1, d2]; field is flat with field[a*d1*d2 + b*d2 + c] (a slowest,
// c fastest) -- the same layout buildGrid uses. Returns index-space geometry.
// layerRange = [a0, a1) optionally restricts polygonization to that cell range on
// the first axis; concatenating the ranges [0,k),[k,m),... over a fixed field
// yields the same triangles as one full pass (vertices are emitted per cell).
export function marchingCubes(dims, field, level = 0, layerRange = null) {
  const [d0, d1, d2] = dims;
  const potential = (a, b, c) => field[a * d1 * d2 + b * d2 + c] - level;
  const [a0, a1] = layerRange ?? [0, d0 - 1];
  // clamp: cells outside [0, d0-1) would read past the field (NaN -> phantom faces)
  const start = Math.max(0, a0);
  const end = Math.min(d0 - 1, a1);
  const { positions, cells } = polygonize(dims, potential, undefined, start, end);
  return { vertices: positions, faces: cells };
}
